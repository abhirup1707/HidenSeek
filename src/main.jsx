  ...
  return (
    <group position={[x, h / 2, z]}>
      <mesh castShadow receiveShadow>
        ...
        <PBR color={tint} roughness={0.67} metalness={0.1} />
      </mesh>
  ...
  return (
    <group>
      <mesh rotation={[-Math.PI / 2, 0, 0]} receiveShadow>
        ...
        <MeshReflectorMaterial blur={[350, 120]} resolution={512} mixBlur={0.72} mixStrength={1.55} roughness={0.27} depthScale={0.8} minDepthThreshold={0.42} maxDepthThreshold={1.15} color="#151a20" metalness={0.42} />
      </mesh>
      {buildings.map((b, i) => <Building key={i} x={b[0]} z={b[1]} w={b[2]} d={b[3]} h={b[4]} tint={i % 3 === 0 ? '#222b35' : '#2c3541'} />)}
      <mesh position={[0, 0.05, 0]} receiveShadow>
        <boxGeometry args={[62, 0.09, 7]} />
        <PBR color="#30363e" roughness={0.82} metalness={0.24} />
      </mesh>
      <mesh position={[0, 0.05, 0]} receiveShadow>
      ...
      {[...Array(7)].map((_, i) => {
        ...
        return <React.Fragment key={v}>
          <mesh position={[v, 0.105, 0]}><boxGeometry args={[0.08, 0.025, 62]} /><meshStandardMaterial color="#b5a46d" emissive="#b5a46d" emissiveIntensity={0.15} /></mesh>
          <mesh position={[0, 0.105, v]}><boxGeometry args={[62, 0.025, 0.08]} /><meshStandardMaterial color="#b5a46d" emissive="#b5a46d" emissiveIntensity={0.15} /></mesh>
        </React.Fragment>;
      })}
    </group>
  );
...
function Rain({ count = 1800 }) {
  ...
  const positions = useMemo(() => {
    ...
    for (let i = 0; i < count; i += 1) {
      a[i * 3] = THREE.MathUtils.randFloatSpread(72);
      a[i * 3 + 1] = Math.random() * 20 + 3;
      a[i * 3 + 2] = THREE.MathUtils.randFloatSpread(72);
    }
    return a;
  }, [count]);
  useFrame((_, delta) => {
    ...
    for (let i = 0; i < count; i += 1) {
      const idx = i * 3 + 1;
      attr.array[idx] = attr.array[idx] - delta * 17 < 0.2 ? 23 : attr.array[idx] - delta * 17;
    }
    attr.needsUpdate = true;
  });
  return <points ref={ref} frustumCulled={false}>
    <bufferGeometry><bufferAttribute attach="attributes-position" args={[positions, 3]} /></bufferGeometry>
    <pointsMaterial color="#c0dbff" size={0.048} transparent opacity={0.42} depthWrite={false} sizeAttenuation />
  </points>;
}
...
function Core({ position, taken, onTake }) {
  ...
  return <group ref={ref} position={position} onClick={onTake}>
    <mesh castShadow><icosahedronGeometry args={[0.3, 3]} /><PBR color="#b9fcff" emissive="#16dfff" emissiveIntensity={5.5} roughness={0.12} metalness={0.6} /></mesh>
    <pointLight color="#18dfff" intensity={8} distance={4.5} />
  </group>;
}
...
function Drone({ position, index, onHit }) {
  ...
  useFrame((state) => {
    ...
    ref.current.position.set(position[0] + Math.sin(t * 0.7 + index) * 4, position[1] + Math.sin(t * 1.3 + index) * 0.65, position[2] + Math.cos(t * 0.6 + index) * 3);
    const p = window.__nightshiftPlayer;
    if (p && (p.x - ref.current.position.x) ** 2 + (p.z - ref.current.position.z) ** 2 < 2.25 && Math.abs(p.y - ref.current.position.y) < 1.8) onHit();
  });
...
function Controller({ bodyRef, active }) {
  ...
  useEffect(() => {
    const down = (e) => { keys.current[e.code] = true; if (['Space', 'ShiftLeft', 'ShiftRight'].includes(e.code)) e.preventDefault(); };
  ...
  useEffect(() => {
    ...
    const click = () => { if (active && document.pointerLockElement !== canvas) canvas.requestPointerLock?.(); };
    const move = (e) => { if (!active || document.pointerLockElement !== canvas) return; yaw.current -= e.movementX * 0.0025; pitch.current = THREE.MathUtils.clamp(pitch.current - e.movementY * 0.0018, -0.55, 0.58); };
    canvas.addEventListener('click', click); document.addEventListener('mousemove', move);
    return () => { canvas.removeEventListener('click', click); document.removeEventListener('mousemove', move); };
  }, [gl, active]);
  useFrame((_, delta) => {
    if (!active || !bodyRef.current) return;
    const body = bodyRef.current, vel = body.linvel(), input = new THREE.Vector3();
    if (keys.current.KeyW) input.z -= 1; if (keys.current.KeyS) input.z += 1; if (keys.current.KeyA) input.x -= 1; if (keys.current.KeyD) input.x += 1;
    if (input.lengthSq()) {
      input.normalize(); const sy = Math.sin(yaw.current), cy = Math.cos(yaw.current); const wx = input.x * cy - input.z * sy; const wz = input.x * sy + input.z * cy; const speed = keys.current.ShiftLeft || keys.current.ShiftRight ? DASH_SPEED : WALK_SPEED;
      body.setLinvel({ x: wx * speed, y: vel.y, z: wz * speed }, true); body.setRotation(new THREE.Quaternion().setFromEuler(new THREE.Euler(0, Math.atan2(wx, wz), 0)), true);
    } else body.setLinvel({ x: 0, y: vel.y, z: 0 }, true);
    const pos = body.translation(), grounded = pos.y <= 1.17;
    if (keys.current.Space && !jumpLatch.current) { jumpLatch.current = true; if (grounded) body.setLinvel({ x: vel.x, y: JUMP_SPEED, z: vel.z }, true); }
    if (!keys.current.Space) jumpLatch.current = false;
    window.__nightshiftPlayer = { x: pos.x, y: pos.y, z: pos.z };
    const target = new THREE.Vector3(pos.x, pos.y + 0.9, pos.z), distance = 6.1;
    const offset = new THREE.Vector3(Math.sin(yaw.current) * distance * Math.cos(pitch.current), 2.4 + Math.sin(pitch.current) * distance, Math.cos(yaw.current) * distance * Math.cos(pitch.current));
    camera.position.lerp(target.clone().add(offset), 1 - Math.pow(0.001, delta)); camera.lookAt(target);
  });
  return null;
}
...
function World({ bodyRef, active, taken, onTake, onDroneHit, round }) {
  return <>
    ...
    <NeonPanel position={[-3, 2.2, -8]} color="#2deaff" />
    <NeonPanel position={[8, 2.1, 5]} color="#ff4e9c" />
    <Suspense fallback={null}><Artifact /></Suspense>
    <Sparkles count={75} scale={[60, 10, 60]} size={1.2} speed={0.1} color="#65dfff" />
...
function App() {
  ...
  const [taken, setTaken] = useState(Array(CORE_POSITIONS.length).fill(false));
  const [hp, setHp] = useState(100);
  const hitLock = useRef(0);

  useFrame((_, delta) => { if (hitLock.current > 0) hitLock.current -= delta; });

  useEffect(() => {
    if (!running || result) return undefined;
    const timer = setInterval(() => setTime((t) => { if (t <= 1) { setResult('TIME'); setRunning(false); return 0; } return t - 1; }), 1000);
    return () => clearInterval(timer);
  }, [running, result]);

  const start = () => { setResult(''); setTime(START_TIME); setTaken(Array(CORE_POSITIONS.length).fill(false)); setHp(100); setRound((r) => r + 1); setRunning(true); };
  ...
  const hitDrone = () => { if (hitLock.current > 0) return; hitLock.current = 1.1; setHp((value) => { const next = Math.max(0, value - 20); if (!next) { setResult('LOSE'); setRunning(false); } return next; }); };
